...
uploaded_file = st.sidebar.file_uploader("Upload consolidated_sales_data.csv", type=["csv"])
...
if uploaded_file is not None:
    ...
    def load_data(file):
        ...
        # Only use months that actually exist in the data to avoid errors
        existing_months = [m for m in month_order if m in df['Month-Year'].unique()]
        df['Month-Year'] = pd.Categorical(df['Month-Year'], categories=existing_months, ordered=True)
        return df
    ...
    executives = ["All"] + sorted(df['Sales Executive'].dropna().unique().tolist())
    selected_executive = st.sidebar.selectbox("Select Sales Executive", executives)

    # Month Slicer
    months = ["All"] + list(df['Month-Year'].cat.categories)
    ...
    with row1_col1:
        ...
        fig_rev.update_layout(yaxis=dict(tickformat=",.0f"), xaxis_title="Month", yaxis_title="Revenue")
    ...
    with row1_col2:
        ...
        fig_rn = px.line(monthly_rn, x='Month-Year', y='Room Nights', markers=True,
                         text='Room Nights', labels={'Room Nights': 'Nights', 'Month-Year': 'Month'})
        ...
        st.plotly_chart(fig_rn, use_container_width=True, theme=None)
    ...
    with row2_col2:
        ...
        st.plotly_chart(fig_arr, use_container_width=True, theme=None)
    ...
    st.info("Please upload the 'consolidated_sales_data.csv' file from the sidebar to begin.")